...
const LLM_URL = (): string => (process.env.GANDALF_LLM_URL ?? "http://127.0.0.1:8787/v1").replace(/\/$/, "");
...
async function diagnose(folder: vscode.WorkspaceFolder, s: Settings): Promise<Diagnosis> {
  ...
  const hasImage = docker ? (await probe(docker, ["image", "inspect", image], folder.uri.fsPath)).ok : false;
