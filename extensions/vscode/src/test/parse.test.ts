...
describe("finding normalization", () => {
  ...
  it("leaves a bracketed prefix that is not a severity word alone", () => {
    const [f] = one(
      gate("bandit", [
        norm(
          { path: "app.py", line: 1, message: "[B603] subprocess call" },
          { path: "app.py", line: 1, message: "[B603] subprocess call" },
        ),
      ]),
    );
    ...
    const [f] = one(
      gate("mypy", [
        norm(
          ...
          {
            path: "src/gandalf/__main__.py",
            line: 465,
            message: 'error: "Gate" has no attribute "langs"  [attr-defined]',
          },
        ),
      ]),
    );
  ...
  it("gives each finding a stable id", () => {
    const build = () =>
      one(gate("ruff", [norm({ path: "app.py", line: 1, message: "x" }, { path: "app.py", line: 1, message: "x" })]))[0]
        .id;
...
describe("a gandalf that predates the normalizer", () => {
  ...
  it("still identifies bandit by its test id", () => {
    const [f] = one(
      gate("bandit", [{ filename: "app.py", line_number: 1, test_id: "B105", issue_text: "pw", code: "1 x\n2 y" }]),
    );
    assert.equal(f.rule, "B105");
  });
...
describe("reported level", () => {
  const levelOf = (severity: string, outcome: RawGate["outcome"] = "warn") =>
    one(
      gate(
        "g",
        [norm({ path: "app.py", line: 1, message: "x" }, { path: "app.py", line: 1, message: "x", severity })],
        {
          outcome,
        },
      ),
...
describe("gates that assessed nothing", () => {
  const bare = () =>
    payloadOf(
      gate("trivy", [], {
        outcome: "warn",
        summary: "trivy unavailable (no host binary or gandalf-tools image) — skipped",
      }),
      ...
      gate("compliance", [], {
        outcome: "warn",
        summary: "compliance: no request/diff to judge (pass --title/--body)",
      }),
