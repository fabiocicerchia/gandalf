...
def _image_part(tools: dict[str, Any], image: int) -> str:
    """`N from <image> (<id>)` — the image is named by content id where it has
    one, since the tag is a moving target."""
    img: dict[str, Any] = tools.get("image") or {}
    ident = str(img.get("id") or "")[:19] or img.get("name", "")
    return f"{image} from {img.get('name', 'image')} ({ident})"


def _sources_line(tools: dict[str, Any], resolved: dict[str, Any]) -> str:
...
def _versions_block(resolved: dict[str, Any]) -> str:
...
def _tools_line(tools: dict[str, Any]) -> str:
    """One-line provenance summary for the terminal footer."""
    resolved: dict[str, Any] = tools.get("resolved") or {}
    if not resolved:
        return ""
    return _sources_line(tools, resolved) + _versions_block(resolved)


def print_summary(run: Run, meta_line: dict[str, Any], cfg: Config, *, explain: bool) -> None:
