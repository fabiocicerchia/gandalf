...
def _markdown(ctx: GateContext) -> list[str]:
    ...
    return [f for f in named(ctx, "*.md") if not any(part.startswith(".") for part in f.split("/"))]
...
class MdlGate:
    ...
    async def run(self, ctx: GateContext) -> GateResult:
        mds = _markdown(ctx)
        if not mds:
            return GateResult(self.name, GateOutcome.PASS, 1.0, "mdl: no markdown files")
        ...
        n = len(issues)
        if n == 0:
            return GateResult(self.name, GateOutcome.PASS, 1.0, f"mdl: {len(mds)} file(s) clean")
