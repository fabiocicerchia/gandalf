...
# Any severe vulnerability fails; this many of lesser severity does too.
MAX_VULNS = 3
MAX_HIGH = 5
# Failing this many scorecard checks is a failure rather than a warning.
MAX_FAILED_CHECKS = 5
...
def _pytest_runner(root: Path, base: list[str]) -> list[str] | None:
    ...
    """
    configured = any((root / f).exists() for f in ("pytest.ini", "pyproject.toml", "setup.cfg"))
...
class OsvGate:
    ...
    async def run(self, ctx: GateContext) -> GateResult:
        ...
        n = len(vulns)
        if n == 0:
            return GateResult(self.name, GateOutcome.PASS, 1.0, "osv: no known vulnerabilities")
        return scored(self.name, n, f"osv: {n} vulnerability(ies)", vulns, fail=n >= MAX_VULNS)
...
class OsvScannerGate:
    ...
    async def run(self, ctx: GateContext) -> GateResult:
        if (m := missing_result(self.name, "osv-scanner")) is not None:
            return m
        rc, out, _ = await run_tool(["osv-scanner", "scan", "--recursive", "--format", "json", "."], ctx.workdir)
        if (to := timeout_result(self.name, rc)) is not None:
            return to
        data = parsed(out)
        if data is None:
        ...
        return scored(self.name, n, f"osv-scanner: {n} vulnerability(ies)", vulns, fail=n >= MAX_VULNS)
...
class TrivyGate:
    ...
    async def run(self, ctx: GateContext) -> GateResult:
        ...
        return scored(
            ...
            fail=n_s > 0 or n_v >= MAX_HIGH or n_m >= MAX_HIGH,
        )
...
class CheckovGate:
    ...
    async def run(self, ctx: GateContext) -> GateResult:
        ...
        total = failed + passed
        if failed == 0:
            return GateResult(self.name, GateOutcome.PASS, 1.0, f"checkov: {passed} check(s) passed")
        ...
        outcome = GateOutcome.FAIL if failed >= MAX_FAILED_CHECKS else GateOutcome.WARN
        return GateResult(
            self.name,
            outcome,
            score,
...
class HadolintGate:
    ...
    async def run(self, ctx: GateContext) -> GateResult:
        ...
        if not dockerfiles:
            return GateResult(self.name, GateOutcome.PASS, 1.0, "hadolint: no Dockerfiles found")
        all_findings, timed_out = await _hadolint_findings(dockerfiles, ctx.workdir)
        if timed_out:
            return unavailable(self.name, f"{self.name}: timed out on {timed_out} — skipped")
        n = len(all_findings)
        if n == 0:
            return GateResult(
                self.name,
...
class TestsGate:
    ...
    async def run(self, ctx: GateContext) -> GateResult:
        ...
        if streams is None:
            return GateResult(self.name, GateOutcome.FAIL, 0.0, "tests: timed out after 120s")
        out_b, _ = streams
        out = out_b.decode(errors="replace") if out_b else ""
        tail = "\n".join(out.strip().splitlines()[-5:])
        if proc.returncode == 0:
            return GateResult(self.name, GateOutcome.PASS, 1.0, f"tests: passed — {tail}")
