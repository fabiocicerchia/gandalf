...
# --- policy -----------------------------------------------------------------
def test_policy_from_config_precedence() -> None:
    p = report.Policy.from_config({"fail_on": "warn", "min_score": 70})
    assert p.fail_on is W
    assert p.min_score == 70
    # CLI overrides file
    p = report.Policy.from_config({"fail_on": "fail"}, cli_fail_on="warn", cli_min_score=50)
    assert p.fail_on is W
    assert p.min_score == 50
...
def test_decide() -> None:
    ...
    assert report.decide(report.Verdict(W, 80), default)[0] is True  # warn passes by default
...
def test_render_terminal_smoke() -> None:
...
def test_render_html_smoke() -> None:
    ...
    html = render_html.render_html(
        ...
        meta={"commit": {"short": "abc123", "subject": "fix"}},
    )
    assert html.startswith("<!DOCTYPE html>")
    assert html.rstrip().endswith("</html>")
    assert "ruff" in html
    assert "1 finding" in html  # findings <details>
...
def test_render_terminal_shows_delta() -> None:
    v = report.aggregate(_results())
    out = render_text.render_terminal("staged", _results(), v, {"summary": "hi"}, {"score_delta": -7})
    assert "(-7 vs prev)" in out


def test_render_html_shows_delta() -> None:
    v = report.aggregate(_results())
    html = render_html.render_html("staged", _results(), v, {"summary": "hi"}, meta={"score_delta": 4})
    assert "(+4 vs prev)" in html


def test_render_html_has_rag_filter_buttons() -> None:
    v = report.aggregate(_results())
...
def test_diff_html_empty_is_blank() -> None:
    assert render_html._diff_html("") == ""
    assert render_html._diff_html("   \n") == ""


def test_diff_html_colors_added_and_removed_lines() -> None:
    diff = "@@ -1,2 +1,2 @@\n-old line\n+new line\n context\n"
    ...
    assert "View raw diff" in out


def test_diff_html_ignores_file_header_markers() -> None:
    diff = "--- a/x.py\n+++ b/x.py\n@@ -1 +1 @@\n-old\n+new\n"
    out = render_html._diff_html(diff)
...
def test_render_html_embeds_diff_view() -> None:
    v = report.aggregate(_results())
    out = render_html.render_html("staged", _results(), v, {"summary": "hi"}, diff="+added line\n")
    assert '<details class="diff-view">' in out
    assert "added line" in out


def test_render_html_without_diff_omits_diff_view() -> None:
    v = report.aggregate(_results())
    out = render_html.render_html("staged", _results(), v, {"summary": "hi"})
    assert '<details class="diff-view">' not in out


def test_aggregate_edges() -> None:
