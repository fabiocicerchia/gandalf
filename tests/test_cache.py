...
# --- key salt + expiry ------------------------------------------------------
def test_salt_changes_the_key(tmp_path):
    ...
    assert plain != salted != other and plain != other


def test_toolchain_salt_tracks_the_image_id(monkeypatch):
...
def test_advisory_gates_expire_but_others_do_not():
    class G:
        name = "trivy"
...
def test_a_gate_may_override_its_own_ttl():
    class Custom:
        name = "trivy"
        cache_ttl = 60
...
def test_a_stale_advisory_entry_is_a_miss():
...
def test_entry_without_a_timestamp_is_expired_when_a_max_age_applies():
